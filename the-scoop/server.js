...
function createComment(url, request) {
    ...
    const comBody = requestComment && requestComment.body;
    const comUsername = requestComment && requestComment.username;
    const comArticleId = requestComment && requestComment.articleId;
    const response = {};

    if (
      comBody &&
      database.users[comUsername] &&
      database.articles[comArticleId]
    ) {
      const comment = {
      ...
      database.comments[comArticleId] = comment;
      database.users[comUsername].commentIds.push(comment.id);
      database.articles[comArticleId].commentIds.push(comArticleId);
...
const getRequestRoute = (url) => {
  ...
  if (pathSegments.length === 1) {
    ...
    pathSegments[2] === 'upvote' ||
    pathSegments[2] === 'downvote'
  ) {
